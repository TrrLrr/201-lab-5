...
function sumAndMultiply(a,b,c){ //eslint-disable-line
  var theNextProduct = multiply(a,b);
  theNextProduct = multiply(theNextProduct[0], c);
  theNextProduct = theNextProduct[0];
  var nextSum = sum(a, b);
  nextSum = sum(nextSum[0], c);
  nextSum = nextSum[0];
  var sumMessage = a + ' and ' + b + ' and ' + c + ' sum to ' + nextSum +'.';
  var productMessage = 'The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + theNextProduct + '.';
  return [nextSum, theNextProduct, sumMessage, productMessage];
}
...
function sumArray(testArray){ //eslint-disable-line
  ...
  for(var i = 0;i < testArray.length; i++) {
    arrSum = sum(arrSum,testArray[i]);
    arrSum = arrSum[0];
    arrMessage += testArray[i];
    if ( i !== testArray.length - 1) arrMessage += ',';
    }

    arrMessage += ' was passed in as an array of numbers, and ' + arrSum + ' is their sum.';
...
function multiplyArray(testArray){ //eslint-disable-line
  var arrProd = 1;
  var prodArrMessage = 'The numbers ';

  for( var i = 0; i < testArray.length; i++) {
    arrProd = multiply(arrProd, testArray[i]);
    arrProd = arrProd[0];
    prodArrMessage += testArray[i];
    if (i !== testArray.length - 1) prodArrMessage += ',';
  }

  prodArrMessage += ' have a product of ' + arrProd + '.';
  return [arrProd, prodArrMessage];
